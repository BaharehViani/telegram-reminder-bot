...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "4️⃣ به چت بات برگردید و «به‌روزرسانی لیست» را بزنید تا گروه/کانال جدید نمایش داده شود.\n"
        "⚠️ نکته: اگر گروه/کانال در لیست ظاهر نشد، مطمئن شوید که هم شما و هم بات همچنان ادمین هستید."
    )
...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    await update.message.reply_text(
        "برای ارتباط با پشتیبانی بات به آیدی زیر پیام دهید.\n"
        f"{SUPPORT_USERNAME}",
        reply_markup=get_main_keyboard()
    )
